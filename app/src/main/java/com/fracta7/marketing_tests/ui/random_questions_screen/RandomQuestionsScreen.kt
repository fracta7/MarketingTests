...
@Composable
fun RandomQuestionsScreen(navController: NavController) {
    ...
    if (viewModel.questions.isNotEmpty()) {
        ...
        Scaffold(topBar = {
            TopAppBar(title = { Text(text = "Random Questions") }, navigationIcon = {
                IconButton(onClick = {
            ...
            Column(modifier = Modifier.padding(it)) {
                LinearProgressIndicator(
                    progress = currentQuestion / 75f, modifier = Modifier.padding(12.dp).fillMaxWidth()
                )
                Text(
            ...
            if (openDialog) {
                AlertDialog(onDismissRequest = {
                    ...
                    Text(text = "You got $correctAnswers out of 75 or ${(correctAnswers / 75f * 100 * 10).toInt() / 10f}%")
                })
            }
        })
    }
